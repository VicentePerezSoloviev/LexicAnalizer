 ...
 */
tipoclave _clave_elem(lexema * E){
	return E->lexema;
}
...
int _comparar_clave_elem(tipoclave cl, lexema E){
...
/* Funcion privada para pasar la clave y no tener que
   ...
   */
void _modificar(abb *A, tipoclave cl, lexema nodo){
    if (es_vacio(*A)) {
        return;
    } 
...
/* Permite modificar el nodo extrayendo del mismo 
   la clave */
void modificar(abb *A, lexema nodo) {
...
unsigned es_miembro(abb A, lexema E) {
...
void buscar_nodo(abb A, tipoclave cl, lexema *nodo) {
    if (es_vacio(A)) {
        return;
    } 
    int comp = _comparar_clave_elem(cl, A->info);
